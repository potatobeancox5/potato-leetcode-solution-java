package com.potato.study.leetcodecn.p01387.t001;
...
public class Solution {
    ...
    public int getKth(int lo, int hi, int k) {
        // 从 lo 到 hi 每个数字 计算它的 权重，放入优先级队列
        ...
        for (int i = lo; i <= hi; i++) {
            priorityQueue.add(new int[] {i, getWeights(i)});
        }
        // 循环 pop 直到 k
        while (k > 1) {
            k--;
     ...
     * @return
     */
    private int getWeights(int val) {
        int step = 0;
        while (val != 1) {
            if (val % 2 == 0) {
                // x 是偶数，那么 x = x / 2
    ...
    public static void main(String[] args) {
        ...
        int hi = 15;
        int k = 2;
        int res = solution.getKth(lo, hi, k);
