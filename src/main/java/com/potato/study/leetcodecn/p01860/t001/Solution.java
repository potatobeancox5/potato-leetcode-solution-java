package com.potato.study.leetcodecn.p01860.t001;
 ...
 *
 * 请你返回一个数组，包含 [crashTime, memory1crash, memory2crash] ，其中 crashTime是程序意外退出的时间（单位为秒）， memory1crash
...
public class Solution {
    ...
    public int[] memLeak(int memory1, int memory2) {
        int timeStamp = 1;
        while (memory1 >= timeStamp || memory2 >= timeStamp) {
            // 大的肯定满足
            if (memory1 >= memory2) {
                memory1 -= timeStamp;
            } else {
        ...
        int[] crash = new int[] {timeStamp, memory1, memory2};
        return crash;
    }
