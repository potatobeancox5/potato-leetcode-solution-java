package com.potato.study.leetcodecn.p01600.t001;
...
public class ThroneInheritance {
    ...
    public ThroneInheritance(String kingName) {
        ...
        // 处理 国王的
        nameListMap.put(kingName, new ArrayList<>());
    }

    // parentName 的孩子出生 导致 继承顺序变化
    public void birth(String parentName, String childName) {
        List<String> nameList = nameListMap.getOrDefault(parentName, new ArrayList<>());
        nameList.add(childName);
        nameListMap.put(parentName, nameList);
    }
    ...
    public List<String> getInheritanceOrder() {
        ...
        while (!deque.isEmpty()) {
            ...
            if (!deathSet.contains(poll)) {
                inheritanceList.add(poll);
            }
            List<String> child = nameListMap.get(poll);
            if (null == child) {
                continue;
            }
            for (int i = child.size() - 1; i >= 0; i--) {
                deque.addFirst(child.get(i));
            }
        }
