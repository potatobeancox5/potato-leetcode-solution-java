package com.potato.study.leetcodecn.p00748.t001;
...
public class Solution {
    ...
    public String shortestCompletingWord(String licensePlate, String[] words) {
        // 统计 licensePlate 字母出现次数 全转化成小写
        int charLength = 0;
        int[] licenseCount = new int[26];
        for (int i = 0; i < licensePlate.length(); i++) {
            char ch = licensePlate.charAt(i);
            if (Character.isAlphabetic(ch)) {
                licenseCount[Character.toLowerCase(ch) - 'a']++;
                charLength++;
            }
        }
        //  对word 进行遍历 长度小的 continue 比当前最优解大的continue （如果有解了） ，统计次数 比较
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            // 比当前最优解大的continue
            if (null != shortWord && shortWord.length() <= word.length()) {
                continue;
            }
            int[] count = getCount(word);
            if (isValid(count, licenseCount)) {
                shortWord = word;
            }
        }
        return shortWord;
    ...
    private boolean isValid(int[] count, int[] licenseCount) {
        for (int i = 0; i < 26; i++) {
            if (licenseCount[i] > count[i]) {
                return false;
            }
        }
        return true;
